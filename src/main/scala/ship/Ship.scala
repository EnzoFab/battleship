...
abstract class Ship{
	...
	/**
		...
		* @param orientation
		*/
	def create(x: Int, y: Int, orientation: String, size: Int): List[Square] = {
		var newX: Int = x
		var newY: Int = y
		...
		if (size == shipSize) Nil
		else {
			Square(x, y, isTouched = false, shipName.charAt(0) + "") :: create(newX, newY, orientation, size + 1)
		}
	}
